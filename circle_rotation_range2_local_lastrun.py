...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Volumes/rdss_kahwang/cicle_rotation_range2_local/circle_rotation_range2_local_lastrun.py',
    ...
    for thisPractice in practice:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if fixation.status == STARTED:
                ...
                if tThisFlipGlobal > fixation.tStartRefresh + 1.3-frameTolerance:
            ...
            if left_gabor.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > left_gabor.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if right_gabor.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > right_gabor.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if fixation2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixation2.tStartRefresh + 1.3-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if left_distractor_prac.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > left_distractor_prac.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if right_distractor_prac.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > right_distractor_prac.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if fixation3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixation3.tStartRefresh + 1.2-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if left_impulse.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > left_impulse.tStartRefresh + 0.1-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if central_impulse.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > central_impulse.tStartRefresh + 0.1-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if right_impulse.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > right_impulse.tStartRefresh + 0.1-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # *fixation4* updates
            if fixation4.status == NOT_STARTED and tThisFlip >= 0.1-frameTolerance:
                # keep track of start time/frame for later
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if fix_cross.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fix_cross.tStartRefresh + 1.1-frameTolerance:
                # keep track of stop time/frame for later
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if fix_cross2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fix_cross2.tStartRefresh + 1.1-frameTolerance:
                # keep track of stop time/frame for later
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if fix_cross3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > fix_cross3.tStartRefresh + 1.1-frameTolerance:
                # keep track of stop time/frame for later
